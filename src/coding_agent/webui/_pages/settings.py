...
def render_settings() -> None:
    ...
    if st.toggle("Browse Memory Records", value=False):
        ...
        st.caption(f"Loaded memory records: {len(records)}")
        for idx, row in enumerate(records):
            record_id = str(row.get("record_id", ""))
            with st.expander(f"{record_id} · {row.get('layer', 'unknown')} · {row.get('status', 'unknown')}", expanded=False):
                st.caption(f"source={row.get('source', '')} · scope={row.get('scope_key', '')} · updated={row.get('updated_at', '')}")
                st.code(str(row.get("content", "")), language="text")
                corrected_content = st.text_area(
                    "Corrected Content",
                    value=str(row.get("content", "")),
                    height=180,
                    key=f"memory_correct_content_{idx}",
                )
                correction_reason = st.text_input(
                    "Correction Reason",
                    value="",
                    key=f"memory_correct_reason_{idx}",
                )
                if st.button("Save Correction", key=f"memory_correct_btn_{idx}", use_container_width=True):
                    new_id = store.store_memory(
                        layer=str(row.get("layer", "project/context")),
                        content=corrected_content.strip(),
                        scope_key=str(row.get("scope_key", "global")),
                        source="settings_memory_correction",
